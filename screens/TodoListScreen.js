...
export default function TodoListScreen() {
    ...
    return (
        <View className="flex-1 flex flex-col items-start justify-start p-7 bg-teal-50">
            <Text className="text-3xl text-[#219ebc] font-bold text-left ">Todo List</Text>
            <ScrollView className="flex-1 w-full my-8 flex flex-col">
                {tasks.map((task, index) => <TodoItem key={index + task} task={task} index={index + 1} onPress={() => completeTask(task, index)} />)}
            </ScrollView>
            {completedTasks.length > 0 && <>
                <Text className="text-3xl text-teal-600 font-bold text-left ">Completed Todo</Text>
                <ScrollView className="flex-1 w-full my-8 flex flex-col">
                    {completedTasks.map((task, index) => <TodoItem isCompleted key={index + task} task={task} index={index + 1} onPress={() => unCompleteTask(task, index)} />)}
                </ScrollView>
            </>}
